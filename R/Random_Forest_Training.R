...
random_forest_training <- function(training_setin){
  ...
  # Finds index of any any row with outliers
  outliers2 <- which(outliers == TRUE, arr.ind = TRUE)
  row_out_ind <- unique(outliers2[,1])

  # remove rows with outliers
  training_rmv_outliers <- training_setin[-row_out_ind,]
