read_in_extended <- function(rc, sys){
  #user chooses
  files <- choose.files()
  ...
  wavenumber <- as.data.frame(seq(250, 3100, by = 1))
  
  signal_max <- matrix(nrow = 6, ncol = 3)
  for (i in 1:nrow(raw_spec)){
    max_i <- which.max(raw_spec[i,700:800]) # taking 1015-300 and 1015-380 due to flipping
    shift_i <- max_i + 699
